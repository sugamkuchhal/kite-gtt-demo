...
def get_instructions_sheet(sheet_id=None, tab_name=None):
    if sheet_id is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sheet-name", dest="tab_name", help="Instruction TAB_NAME (overrides config.INSTRUCTION_TAB_NAME)", type=str)
    ...
    if not tab_name:
        missing.append("sheet-name (or config.INSTRUCTION_TAB_NAME)")
    if missing:
